...
### 2026-08-20 — YouTube Player Hardening

Same code-review pass that surfaced the notes-buffer regression also flagged the YouTube player as an in-app browser risk. `YouTubePlayerView` shipped with three defaults that would each be one bad tap away from a real bug: persistent cookies/login state via the default `WKWebsiteDataStore`, no `WKNavigationDelegate` at all (so any tap on YouTube's own links — channel, related, share, sign-in — navigated in-frame inside a rounded 16:9 window with no back button), and raw string interpolation of `videoID` into the URL (safe for legit 11-char base64url IDs, but `youtubeVideoID` originates in unvalidated ADB JSON).

Fixed all three:

1. **Ephemeral data store** — `config.websiteDataStore = .nonPersistent()`. No cookies, cache, or login state persists between launches or leaks into other future WKWebViews in the app. Repair-log viewing has zero reason to inherit or write to the user's YouTube session.

2. **Navigation delegate** — new `Coordinator: NSObject, WKNavigationDelegate`. The async policy method checks `navigationType`: anything that isn't `.linkActivated` or `.formSubmitted` (initial load, YouTube's own AJAX, fullscreen transitions) stays in-frame. User-driven link taps are inspected — if the target URL is the same YouTube video (host ends in `youtube.com`/`youtu.be` AND `?v=` matches `allowedVideoID`), it's allowed. Everything else hands off to `UIApplication.shared.open(url)`, so the user leaves via Safari instead of getting stuck in a stripped-mobile-YouTube session.

3. **Regex-validated ID + `URLComponents`** — `YouTubePlayerView.isValid(id:)` checks against `^[A-Za-z0-9_-]{6,32}$` (11 is canonical, widened to 6–32 for future format tolerance without opening injection-friendly punctuation). Applied at two layers: `resolvedYouTubeID` at the caller filters out garbage before the section renders, and `makeUIView` re-checks defensively before loading. The URL itself is now built via `URLComponents` + `URLQueryItem` — self-documenting and correct even if someone later adds a query param.

Also added `dismantleUIView(_:coordinator:)` to explicitly `stopLoading()`, `removeAllUserScripts()`, and null out the navigation delegate. WKWebView deallocation would clean up the JS context anyway (including the `setInterval(fitVideo, 500)` the review flagged), but explicit cleanup is cheap insurance and future-proofs against SwiftUI holding the view alive longer than expected during transitions.

**Swift 6 tax, once again**: after adding `nonisolated static func isValid`, the build warned that `Self.idPattern` was main-actor-isolated. The static regex property needed the same treatment (`nonisolated static let idPattern`). Same lesson as `Bundle.appVersion` and `ArtworkKind.bulkFetchable` in the July reuse pass — static properties on main-actor types inherit isolation, and any nonisolated method that reads them silently drags in the same warning.
